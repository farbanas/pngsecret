...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk, Error> {
        ...
        let index = match index_opt {
            Some(index) => index,
            None => return Err(Error::new(io::ErrorKind::Other, "chunk doesn't exist")),
        };
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        ...
        if header != Png::STANDARD_HEADER {
            return Err(Error::new(io::ErrorKind::Other, "header is incorrect"));
        }

        let mut rest: Vec<u8> = vec![];
        buf_reader.read_to_end(&mut rest)?;

        let mut rest_bytes = rest.as_slice();
        ...
        loop {
            ...
            let chunk = Chunk::try_from(rest_bytes)?;
            let size = chunk.length() + 4 + 4 + 4;
            rest_bytes = &rest_bytes[size as usize..];

            chunks.push(chunk);
        }

        let png = Png { header, chunks };
...
impl Display for Png {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let body_str: Vec<String> = self.chunks.iter().map(|chunk| chunk.to_string()).collect();

        f.write_str(&body_str.join(""))
    }
}
